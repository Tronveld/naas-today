...
**The approved-events query lives in `netlify/functions/lib/events-query.js`** (ESM; esbuild converts it for `require`). `approvedEventsQuery(today)` is read by both `get-events.js` and `index.astro`'s build, and returns only events that have not ended (`date >= today` or `end_date >= today`). **Past days are not viewable** — a product decision (2026-09-23): the date picker's `min` is today and `readUrlState` ignores a past `?date=`. Before this, every visit shipped all 887 approved rows twice, 450 of them over. `dublinToday()` is the one place "today in Naas" is computed server-side.

**Recurring dates live in `netlify/functions/lib/recurrence.js`**, shared by `submit-recurring.js` and the submit form's "~N occurrences" hint. UTC arithmetic on `YYYY-MM-DD` strings, so the host's time zone cannot shift a row; monthly clamps to short months (Jan 31 → Feb 28 → Mar 31); a multi-day event keeps its span on every occurrence. Each of those was a live bug until 2026-09-23 — see `tests/recurrence.test.js`.
