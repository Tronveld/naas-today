...
- ~~Filter by all six categories.~~ **Built, but no longer shown** (decided 2026-08-09). The chips, their live counts, the AND combination, the shareable `?filters=` URL and the "Show all events" escape are all intact behind `FILTERS_ENABLED` in `src/scripts/flags.js`; flipping that one boolean restores the row whole. Measured over the 60 days from 2026-08-09: only **1.77 of six chips were live on an average day**, Theatre was live on 5 days and Music on 12, and just 22 of 60 days carried the 3+ events it takes for a filter to narrow anything — while the row cost ~122px above the fold on all 60. The per-card category tags carry the same information for the day in front of the reader, which on a one- or two-event day is quicker than a control. **The parents-with-children case is served by the tags, not by the chips.** Whether to bring the row back, in some conditional or collapsed form, is open.
...
- ~~**There is no answer to "what's on this weekend?" beyond stepping day by day.**~~ **Largely closed 2026-08-10 by the week strip.** The Users section names this as one of the two defining questions and for a long time the interface answered only the other one. It was reviewed on 2026-08-07 (critique item 70) and deliberately left alone, on the grounds that a second navigation mode deserved its own design pass rather than arriving as a side effect of a refactor.

  It got that pass. The seven-day strip under the band shows each of the next seven days with its real event count, so "is anything on this weekend" is answered without a single tap — and the counts are pre-rendered, so they are right before JS runs. **Partly reopened 2026-09-23:** the counts were replaced by dates (a count under a weekday read as a date), so the weekend question now takes a tap per day. Any event mark that comes back has to read as "something on" and not as a number. The day-at-a-time shape is intact underneath: the strip navigates *to* a day, it does not replace the day view with a week view. That distinction was an explicit owner decision during the redesign, made against a rendered week-view alternative that was rejected.

  **Closed 2026-09-23 — dates past the week.** The strip covers seven days, and anything beyond them was the footer's date picker (the owner had chosen "keep it simple — footer only"). The owner reversed that: planning next week is a common visit, and the link sat below Coming up where nobody found it. The strip now ends in a fixed Later slot that opens the picker and shows the date when the day being viewed is past the seven. The strip still does not page, and there is still no week view.

**Closed 2026-08-09 — the chrome above the first event card.** It had been measured at ~277px of a 375×812 viewport (masthead 54, date section 101, filter rows 122), recorded here as decided-but-not-closed, and noted as needing a layout answer rather than another spacing trim. It got one: the filter row is no longer rendered, and the masthead and date bar became a single nameplate instead of two display blocks at nearly the same size. Now ~152px on today (where the date nav is two buttons) and ~208px on a navigated day (four buttons, 2×2). Kept here as the record of a gap that was closed, not as an open item.

**Superseded 2026-08-10.** The nameplate is gone and the number is now deliberately two numbers, because the band is sized by its own answer rather than by its own importance. On a day with events it is one line over a 44px week strip — around 110px before the first card, less than either predecessor while showing the count, the date and seven days of navigation. On a day with nothing on it is larger on purpose: there are no cards to push down, and the space buys the next real event by name instead of a blank screen. **Treat "chrome above the fold" as answered; do not reopen it as a spacing budget.**

  **Revised 2026-09-23.** On a busy day the band lost its sentence ("Four things on today." restated the strip's count directly below it) and is now one row, brand and date, with the date promoted to be read. The strip lost its "Week ahead" label (80px that left each day a 24px target) and shows each day's date instead of its count, because "Sat 6" read as Saturday the 6th; it carries no event mark. The empty day's answer moved out of the band too: "Nothing on Monday." now sits where the cards would be, with Coming up under it, and the band's "Next in Naas is tomorrow at 10am" line is gone — the first Coming up row is that event, and "tomorrow" read as tomorrow-from-today. About 123px before the first card on every day. **The strip no longer answers "is anything on this weekend?" without a tap**; that is the known cost.
- **Filters AND together.** Two lit chips means an event must match both. This is stated on screen when a filtered day comes up empty with more than one chip active (critique item 71), rather than being left to inference. Whether OR would serve the "free things for the kids" case better is untested. Dormant while the chips are not rendered.
- **The current day is a date, not a clock.** `renderEvents()` filters on the date and sorts ascending by time, so at 8pm the top of today's answer is a parkrun that finished at 10am. Known, unfixed, and deliberately not folded into the 2026-08-09 redesign: the fix is to partition the *current* day at `now` with a quiet "Earlier today" divider, **not** to hide past events, because an event with no end time may still be running. **Recorded here so the gap stays visible rather than being rediscovered as a bug.**
...
- **Binding visual constraints already in force:** hedgerow green `#186C42` as the brand anchor, carried as a *field* rather than an accent; a near-white page; the system UI stack for everything, with tabular figures where numbers align; **light mode only**.

  **Changed 2026-08-10, deliberately.** This line used to bind forest green `#2d5a2d`, warm beige/linen backgrounds, and Georgia + system UI + DM Mono. The owner released all of it for the homepage redesign, keeping only WCAG AA, product truth, copy and function — see `.impeccable/mocks/README.md` for the rounds that followed and the eight worlds that were rejected on the way. What survived is not an inheritance: green is still the brand voice because the chosen direction wanted a saturated field and green is the one this town's site has always spoken in, and light-only survived on its own merits. Georgia, DM Mono and the beige did not survive, and the six category colours went with them. **The three-dependency budget now extends to fonts: there is no webfont at all.**
